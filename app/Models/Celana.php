...
class Celana extends Model
{
    ...
    public static function getCelanaList($payload)
    {
        $query = self::with([
            'item.firstImage',
            'item.subcolor.color'
        ...
        $celanas = $query->orderBy($sort, 'desc')
            ->paginate($limit)
            ->through(function ($celana) {
                return [
                    'id' => $celana->id,
                    'code' => $celana->item->code,
                    'name' => $celana->item->name,
                    'color' => $celana->item->subcolor->color->name,
                    'subcolor' => $celana->item->subcolor->name,
                    'production_month' => $celana->item->production_month,
                    'production_year' => $celana->item->production_year,
                    'image_url' => asset('storage/' . $celana->item->firstImage?->image_url),
                ];
            });
    ...
    public static function getCelanaById($id)
    {
        $query = self::with([
            'item.firstImage',
            'item.subcolor.color'
